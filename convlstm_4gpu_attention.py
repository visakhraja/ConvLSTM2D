...
import pandas as pd
import xarray as xr
from tensorflow.keras.layers import Input, ConvLSTM2D, Reshape, LeakyReLU, Conv2D, BatchNormalization, MultiHeadAttention, Flatten, Dense
...
time, height, width, channels = input_shape

def build_model_with_attention(input_shape):
    inputs = Input(shape=input_shape)
    
    # Initial ConvLSTM2D and normalization layers as in your existing model
    x = ConvLSTM2D(filters=64, kernel_size=(3, 3), padding='same', return_sequences=True)(inputs)
    x = BatchNormalization()(x)
    x = ConvLSTM2D(filters=16, kernel_size=(5, 5), padding='same', return_sequences=True)(x)
    x = BatchNormalization()(x)

    # Flatten the output to fit the MultiHeadAttention input requirements
    x = Reshape((time, -1))(x)
    feature_dim = height * width * channels // time
    # Adding MultiHeadAttention
    # Note: You need to adjust the dimensions (e.g., feature_dim) according to your specific model
    #query_encoding = Dense(feature_dim)(x)
    #value_encoding = Dense(feature_dim)(x)
    attention_output = MultiHeadAttention(num_heads=2, key_dim=512)(x, x)
    x = Reshape((7, 240, 440, 16))(attention_output)
    x = ConvLSTM2D(filters=1, kernel_size=(3, 3), padding='same', return_sequences=False)(x)
    #x = Reshape((240, 440, 4))(attention_output)  # Adjust `desired_shape` as needed
    x = Conv2D(filters=16, kernel_size=(3, 3), activation='relu', padding='same')(x)
    x = Conv2D(filters=1, kernel_size=(3, 3), activation='relu', padding='same')(x)
    outputs = Reshape((lats, lons))(x)  # Final output shape adjustment
...
checkpoint_path = "/p/scratch/share/sivaprasad1/visakh/model_checkpoint_attention_1"
...
# set_global_policy('float32')
#save the model 
model.save('convlstm_model_attention_1')

# #save model weights
model.save_weights('convlstm_model_weights_attention_1')
